...
const updateButtonStates = () => {
  ...
  navBtnNext.style.opacity =
    currentSlide === carouselSlides.length - 1 ? "0.3" : "1";
...
let scrollTimeout;
carouselV1Container.addEventListener(
  "wheel",
  (e) => {
...
const carouselSlidesV2 = Array.from(
  document.querySelectorAll(".carousel-slide-v2"),
);
...
const carouselV2Container = document.querySelector(
  ".carousel-track-container-v2",
);
...
const updateButtonStatesV2 = () => {
  ...
  nextBtnV2.style.opacity =
    currentSlideV2 === carouselSlidesV2.length - 1 ? "0.3" : "1";
...
carouselTrackV2.addEventListener(
  "touchstart",
  (e) => {
...
carouselV2Container.addEventListener(
  "wheel",
  (e) => {
    ...
    if (Math.abs(deltaX) > 5 || Math.abs(deltaY) > 5) {
      ...
      scrollTimeoutV2 = setTimeout(() => {
        if (Math.abs(accumulatedDeltaX) > 50 && !isAnimatingV2) {
          if (
            accumulatedDeltaX > 0 &&
            currentSlideV2 < carouselSlidesV2.length - 1
          ) {
            goToSlideV2(currentSlideV2 + 1);
...
const updateScrollytelling = () => {
  ...
  storySteps.forEach((step, index) => {
    ...
    if (
      scrollPosition + windowHeight / 2 >= stepTop &&
